...
import { EmptyIcon, EmptyState, EmptyTitle } from "../../ui/ListingUI";
import useNavigateBack from "../../hooks/useNavigateBack";
import { useRoom } from "../roomDetails/useRoom";
...
export default function EditRoom() {
  ...
  const { isPending: isUpdating, updateRoom } = useUpdateRoom();
  ...
  const onSubmit = async (data) => {
    ...
    formData.append("location", structuredLocation.district || data.location);
    formData.append("area", structuredLocation.localLevel || data.area);
    formData.append("structuredLocation", JSON.stringify(structuredLocation));
    ...
    updateRoom({ id: room.id, formData });
  };

  useEffect(() => {
    //Async fetch happens here

    if (room) {
      console.log(room);
      reset(room); // Populate form with fetched data
      setValue("additionalInfo", room.additionalInfo?.join(", "), {
        shouldDirty: true,
        shouldValidate: true,
      });
      ...
      if (room.geoLocation?.coordinates?.length === 2) {
        setCoordinates({
          lng: room.geoLocation.coordinates[0],
          lat: room.geoLocation.coordinates[1],
        });
      }
    }
  }, [room, reset, setCustomAmenities, setIsOpenCustomAdder, setValue]);
  ...
  if (isError)
    return (
      <EmptyState>
        <EmptyIcon>🥲</EmptyIcon>
        <EmptyTitle>No room found to edit!</EmptyTitle>
      </EmptyState>
    );
  ...
  return (
    <EditRoomContainer>
      <FormHeader>
        <FormTitle>Edit Room Listing</FormTitle>
        <FormSubtitle>
          Make changes to your room listing and update it.
        </FormSubtitle>
      </FormHeader>
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <FormSection>
          <SectionTitle>Location in Nepal</SectionTitle>
          <LocationSelector
        ...
        <FormSection>
          ...
          <label htmlFor="thumbnail-upload">
            <ImageUploadSection hasError={!!errors.thumbnail}>
              <UploadIcon>
                <FiUpload />
              </UploadIcon>
        ...
        <FormSection>
          ...
          <label htmlFor="image-upload">
            <ImageUploadSection hasError={!!errors.images}>
              <UploadIcon>
                <FiUpload />
              </UploadIcon>
        ...
        <FormActions>
          ...
          <Button type="submit" className="primary" disabled={isUpdating}>
            {isUpdating ? <SpinnerMini /> : "Update Room"}
          </Button>
        </FormActions>
      </Form>
    </EditRoomContainer>
