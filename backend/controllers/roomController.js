...
export const normalizeRoomLocation = (req, res, next) => {
  const structuredLocation = parseJSONField(req.body.structuredLocation);
  const coordinates = parseJSONField(req.body.coordinates);

  if (structuredLocation && typeof structuredLocation === "object") {
    req.body.structuredLocation = structuredLocation;
  }
  ...
  if (coordinates && typeof coordinates === "object") {
    ...
    if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
      return next(new AppError("Please provide valid room coordinates.", 400));
    }

    if (!isInsideNepal(lat, lng)) {
      return next(new AppError("Room location must be inside Nepal.", 400));
    }
...
export const getNearbyRooms = asyncHandler(async (req, res, next) => {
  ...
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return next(new AppError("Please provide lat and lng query params.", 400));
  }
...
export const getRoomsInBounds = asyncHandler(async (req, res, next) => {
  ...
  if (![north, south, east, west].every(Number.isFinite)) {
    return next(
      new AppError("Please provide north, south, east and west.", 400),
    );
  }

  const filter = {
    status: "active",
    "geoLocation.coordinates.1": { $gte: south, $lte: north },
    "geoLocation.coordinates.0": { $gte: west, $lte: east },
  };

  if (req.query.type) filter.type = req.query.type;
  if (req.query.provinceId)
    filter["structuredLocation.provinceId"] = Number(req.query.provinceId);
  if (req.query.districtId)
    filter["structuredLocation.districtId"] = Number(req.query.districtId);
  if (req.query.localLevelId)
    filter["structuredLocation.localLevelId"] = Number(req.query.localLevelId);
  if (req.query.ward)
    filter["structuredLocation.ward"] = Number(req.query.ward);

  const doc = await Room.find(filter).limit(limit);
